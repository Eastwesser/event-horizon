...
func (p postgresView) DSN() string {
	return "postgres://" + p.user + ":" + p.password + "@" + p.host + ":" + p.port + "/" + p.database + "?sslmode=disable"
}
...
type billingView struct{ addr string }

func (b billingView) Addr() string { return b.addr }
...
func (c *Config) BillingConfig() BillingConfig {
	return billingView{addr: c.BillingAddr}
}

func (c *Config) PaymentConfig() PaymentConfig {
	return billingView{addr: c.PaymentAddr}
}
