...
### Утро: Настройка проекта
- [ ] Создать React приложение
- [ ] Установить зависимости (см. FRONTEND_INSIGHTS.md)
- [ ] Настроить прокси для API (vite.config.ts)
- [ ] Настроить маршруты (react-router-dom)

### День: Компоненты
- [ ] **Auth компоненты** (Login, Register, JWT storage)
- [ ] **Гексагональная сетка** (HexGrid.tsx) — координаты, отрисовка
- [ ] **Поднос** (Tray.tsx) — 3 случайные стопки
- [ ] **Drag-n-drop** — перетаскивание с подноса на поле
- [ ] **Логика сложения блинов** — проверка соседей, объединение
- [ ] **Пуф-анимация** — исчезновение стопки ≥10, +очки
- [ ] **Счёт и уровень** — обновление UI
- [ ] **Leaderboard (WebSocket)** — подключение, отображение топа
- [ ] **Billing UI** — баланс лампочек/билетиков

### Вечер: Интеграция и тестирование
- [ ] Подключение к Gateway API
- [ ] Отправка рекорда по окончании партии
- [ ] Сквозной тест: регистрация → игра → рекорд → топ → награда
...
# Установка зависимостей
npm install axios react-dnd react-dnd-html5-backend react-router-dom @tanstack/react-query zustand
...
Войти
Сыграть одну партию
Увидеть свой рекорд в топе (через WebSocket)
Получить награду (лампочки/билетики)