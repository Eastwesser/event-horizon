...
go run sol.go --timeout=5s <host> <port>
Пример: go run sol.go --timeout=3s towel.blinkenlights.nl 23. Ctrl+D — закрыть запись и завершить; закрытие сервером — завершение клиента.